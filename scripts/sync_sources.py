...
def sync(output_dir: Path, drivers_url: str = DRIVERS_URL, rmm_url: str = RMM_URL) -> dict[str, str | int]:
    drivers_payload = fetch(drivers_url)
    rmm_payload = fetch(rmm_url)
    drivers = driver_rows(drivers_payload)
    domains = rmm_rows(rmm_payload)
    ...
    return {"driver_hash_rows": len(drivers), "rmm_domain_rows": len(domains)}
...
def main() -> int:
    ...
    try:
        summary = sync(args.output_dir, args.drivers_url, args.rmm_url)
    except Exception as exc:  # noqa: BLE001 - CLI should show a concise failure and non-zero status.
        print(f"sync failed: {exc}", file=sys.stderr)
        return 1
    print(f"synced {summary['driver_hash_rows']} unique driver hashes and {summary['rmm_domain_rows']} RMM rows")
    return 0
